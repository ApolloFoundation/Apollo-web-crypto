...
function is_overflow(x) {
  return (
    (x[0] > P26 - 19 && (x[1] & x[3] & x[5] & x[7] & x[9]) === P25 && (x[2] & x[4] & x[6] & x[8]) === P26) || x[9] > P25
  );
}
...
function c255lmul8h(a7, a6, a5, a4, a3, a2, a1, a0, b7, b6, b5, b4, b3, b2, b1, b0) {
  ...
  r[7] =
    (v = ((v / 0x10000) | 0) + a0 * b7 + a1 * b6 + a2 * b5 + a3 * b4 + a4 * b3 + a5 * b2 + a6 * b1 + a7 * b0) & 0xffff;
...
function core(Px, s, k, Gx) {
  ...
  if (s !== null) {
    ...
    if (is_negative(t1) !== 0) /* sign is 1, so just copy  */ cpy32(s, k);
